# Knowledge Stream - VS Code und PowerShell - Tools und Erweiterungen für Azure

## Tooling für Azure

- [Visual Studio Code](https://code.visualstudio.com/)
- [PowerShell 7](https://github.com/PowerShell/PowerShell/releases/latest)
- [Windows Terminal](https://aka.ms/terminal)
  - [Windows Terminal als Release Datei](https://github.com/microsoft/terminal/releases/tag/v1.6.10571.0)
- [Az Module](https://docs.microsoft.com/en-us/powershell/azure/install-az-ps?view=azps-5.5.0#install-for-current-user)
- [Git](http://git-scm.com/)
- **Extensions für VS Code:**
  - [ARM Params Generator](https://marketplace.visualstudio.com/items?itemName=wilfriedwoivre.arm-params-generator)
  - [ARM Template REPL](https://marketplace.visualstudio.com/items?itemName=shenglol.vscode-arm-template-repl)
  - [Azure Resource Manager (ARM) Tools](https://marketplace.visualstudio.com/items?itemName=msazurermtools.azurerm-vscode-tools)
  - [Bracket Pair Colorizer 2](https://marketplace.visualstudio.com/items?itemName=CoenraadS.bracket-pair-colorizer-2)
  - [GitLens — Git supercharged](https://marketplace.visualstudio.com/items?itemName=eamodio.gitlens)
  - [PowerShell](https://marketplace.visualstudio.com/items?itemName=ms-vscode.PowerShell)
  - [ARM Template Viewer](https://marketplace.visualstudio.com/items?itemName=bencoleman.armview)

## PowerShell Cake - Schönes Terminal:

- [Oh-my-posh](https://ohmyposh.dev/)
- [Cascadia Code PL Font](https://github.com/microsoft/cascadia-code/releases)
- [Posh Git](https://github.com/dahlbyk/posh-git)
